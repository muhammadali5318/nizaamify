...
- **Profit display had a Class-1 bug** (2026-05-12 P0). Two independent profit code paths — `SaleDetailPage.tsx` per-line cell and `monthly_summary.gross_profit` view (Dashboard / Reports) — both ignored the invoice-level `sale_discount_amount`; the view additionally ignored line discounts. Stage 1 (this commit) applied a minimum allocation-side fix in the JS cell only: pre-compute each line's pro-rata share of `sale_discount_amount` via largest-remainder and subtract it from revenue before computing per-line profit. **The full hardening — a single `invoice_financials` view as the source of truth, three new financial audit queries, decimal-precision audit, transaction-integrity ADR, no-JS-Number-on-money audit — folds into v2.6 (Stage 2).** Until that lands, the Dashboard MTD figure remains over-stated. Root cause in `decisions/2026-05-12-profit-calculation-bug-root-cause.md`.
