...
- **v2.9 RBAC** — 20 ADRs filed under `decisions/v29-rbac-INDEX.md`. Permission-based (not role-based) RBAC; 50-permission catalog across 8 categories; three system presets (owner / manager / salesperson) as starting templates with unlimited per-user overrides. Founding owner has every permission via implicit shortcut in `user_has_permission()`. 29 dependency rules in `permissions_catalog.requires` enforced symmetrically at grant + revoke time. Cost-bearing views project conditionally via materialized CTE; 41 existing v2.8.5 RPCs wrapped via rename-and-wrap (`<name>_v28` preserved, outer wrapper does permission + audit `_by_user_id` UPDATE). `current_active_shop_id()` reads the `app-shop-id` header per call (no Supabase Pre-request hook — Free tier fallback). Invitation flow uses 4-digit verbal code + 24h expiry + 5-strike auto-cancel; invitation row snapshots resolved permissions JSONB (authoritative at accept time). Audit log via `user_shop_permission_audit` table. Server-side shipped 2026-05-13; client-side UI is v2.9.1. **Three rules cover most v2.9 decisions:** (1) Owner never has `user_shop_permissions` rows (implicit shortcut). (2) `<name>_v28` inner functions are reachable only via their wrappers — don't delete or inline without a planned cleanup migration. (3) The `app-shop-id` HTTP header is the active-shop transport; clients must inject it via `customFetch`. Audit/design docs in `audit/2026-05-13-rbac-pre-design-audit.md` + `design/2026-05-13-rbac-*.md`.
- **v2.9.1 RBAC frontend** — 14 ADRs filed (5 retroactive hot-patches for migrations 0082-0086, 7 for B.4-B.10 design decisions, 2 closeout). Shipped 2026-05-13 in 8 D.11 clusters: infrastructure (customFetch + permission hooks + guards + PermissionGated) → TopBar+ShopSwitcher → `/settings/team` → `/invite/accept` → POS gates → Products/Customers/Inventory CRUD → Suppliers/Expenses/Reports/Targets → Settings. Migrations 0087 (11 new RPCs: update_customer/product/variant/supplier + archive_product/supplier + get_active_shop + get_shop_settings + 2 team list-RPCs + 1 team-read DEFINER helper) + 0088 (get_invitation_for_acceptance, pre-shop) + 0089 (update_product extension with is_scan_only / has_batches guards / products.type sync — task #23 closeout). 26 routes gated; 7 direct-write hooks migrated to RPCs with `_by_user_id` audit coverage. Auth-gated UI follows B.2 hybrid: POS internals HIDE / management CRUD GREY-OUT with `Requires permission: {key}. Contact your shop owner.` tooltip / owner-only settings sections HIDDEN. **Four rules cover most v2.9.1 decisions:** (1) Audit gaps block flag flips — frontend hooks still on direct-write are release-blocking before any non-owner traffic at a pilot shop (see [[feedback-audit-gaps-block-flag-flip]] memory rule). (2) Synthetic Postgres tests use `SET ROLE authenticated` against real session JWTs only; never `set_config('request.jwt.claims', ...)` shortcuts. (3) Error dispatch by message text via central `src/lib/errorMap.ts` (~144 entries); `_v28` inner bodies are untouched per the v2.9 wrap-and-rename rule. (4) Two-step invitee flow: signup at `/signup` first, then `/invite/accept` — programmatic password-set-during-accept needs admin API, deferred to v2.10. `RBAC_TEAM_UI_ENABLED` env flag flips per-shop manually for the first 1-2 pilot shops; remove flag entirely after 30 days stable. Phase E.3 coverage matrix at `audit/2026-05-13-v291-phase-e-coverage-matrix.md` (250 rows + 6 multi-step flows + 11 halt criteria); MCP-runnable AQ suite + 0086 ledger invariant all return zero. Audit/design docs in `design/2026-05-13-v291-*.md` + `audit/2026-05-13-v291-*.md`.
- **v2.9.2 column-grant security pass** — 2 ADRs filed (`2026-05-13-v292-definerize-financials-and-column-revoke` + `2026-05-13-v292-naming-collision-with-returns-feature`) plus 1 discipline ADR (`2026-05-13-multi-target-authorization-rule`). 4 migrations 0092-0095 — closes raw-API cost-column leaks acknowledged in mig 0091's header. **Three targets shipped**, 2 deferred: (T1) `sale_items.cost_at_sale` — mig 0092 DEFINER-izes `sale_item_financials` + `invoice_financials` with row-filter + conditional NULL projection; mig 0093 applies the column-grant pattern (`revoke select on table from authenticated; grant select (each non-cost column) to authenticated`). (T2) `customers.outstanding_balance` — mig 0094 same column-grant pattern; `customers_view` already projects conditionally. (T3) `inventory_batches.cost_per_unit` — mig 0095 same; `inventory_batches_view` already projects conditionally. (T4–T5) products/variants cost + purchase_items cost — deferred to v2.9.3 post-pilot. **Three rules cover v2.9.2:** (1) Column-level REVOKE is a no-op when table-level GRANT exists — verified the hard way; must `REVOKE SELECT ON table FROM authenticated; GRANT SELECT (col1, col2, ...) ON table TO authenticated` with safe-column enumeration. (2) Any future column added to `sale_items` / `customers` / `inventory_batches` MUST be explicitly added to the per-column `GRANT SELECT` list or it's invisible to the application. (3) Cost-bearing reads route through DEFINER views (`sale_item_financials`, `customers_view`, `inventory_batches_view`), never raw `.from('table').select('cost_col')`. **NAMING NOTE:** authored as "v2.10a/v2.10b" before the namespace collision with v2.10 (returns/refunds/warranty feature ticket) was reconciled; renamed in source. Production `supabase_migrations.schema_migrations` retains v210/v210b names. See ADR for the rename rationale + the multi-target authorization-discipline ADR.
...
- **v2.9** — server-side permission-based RBAC. 50-permission catalog × 8 categories; three presets (owner / manager / salesperson) as starting templates + unlimited per-user overrides. 29 dependency rules. 41 v2.8.5 RPCs wrapped via rename-and-wrap pattern (`<name>_v28` preserved). 14 permission-conditional DEFINER views. Invitation flow with 4-digit verbal code mitigation. Migrations 0068–0080 + cleanup migration 0081 (ADR #23) — consolidated permissive policies on `profiles` + `units_of_measure`, clearing all 10 `multiple_permissive_policies` advisor lints; AQ-23 added to the audit suite (DEFINER-wrapper shape-drift detector; full 65-wrapper baseline = 11 exempt + 54 conforming + 0 deviations). 20 ADRs + 3 bonus in `decisions/v29-rbac-INDEX.md`. Hot-patch chain 0082-0086 (revert profiles policy recursion, drop team-read inline EXISTS, fix `current_active_shop_id()` grouping/`min(uuid)` bugs, ledger audit-at-INSERT for the append-only path) shipped 2026-05-12 under real authenticated traffic. **Client-side UI is v2.9.1.**
- **v2.9.1** — RBAC frontend shipped 2026-05-13. customFetch wrapper injects `app-shop-id` header; permission hooks (`usePermission`, `useSelfPermissions`, `useIsOwner`, `PERMISSION_KEYS`) drive UI gates; 3 new route guards (`RequirePermission`, `RequireAnyPermission`, `RequireOwner`). 26 routes gated; 7 write hooks migrated to RPCs for `_by_user_id` audit coverage. Migrations 0087-0089 + 12 new RPCs (11 in 0087 + 1 in 0088 — `get_invitation_for_acceptance` for pre-shop accept page; 0089 extends `update_product` with is_scan_only/has_batches/products.type-sync — task #23 closeout). 2 new pages (`/settings/team` with 8 dialogs + 50-toggle permission editor; `/invite/accept/:invitation_id` two-column with shop branding per B.4). AQ-23 exempt list extended to 14 entries adding `get_active_shop`, `get_shop_settings`, `get_invitation_for_acceptance`. AQ-24 added (legacy `current_shop_id()` baseline allowlist — 38 `_v28` + 2 pre-v2.9 views; new SQL must use `current_active_shop_id()`). Phase E coverage matrix at `audit/2026-05-13-v291-phase-e-coverage-matrix.md`. **Pre-pilot readiness checklist gates the flag flip** — see `decisions/2026-05-12-v2-9-0-1-frontend-sweep.md` §Discipline lesson + [[feedback-audit-gaps-block-flag-flip]] memory rule. `RBAC_TEAM_UI_ENABLED` env var flips per-shop manually; remove after 30 days stable.
- **v2.9.2** — column-grant security pass shipped 2026-05-13. 4 migrations 0092-0095, 3 of 5 targets shipped; targets 4-5 deferred to v2.9.3 post-pilot. Closes raw-API leaks on `sale_items.cost_at_sale`, `customers.outstanding_balance`, `inventory_batches.cost_per_unit`. Pattern: column-grant (revoke table-level SELECT + re-grant per safe-column) + DEFINER-ized financial views. Verified live: `SET ROLE authenticated; SELECT <cost_col> FROM <table>` → 42501 across all three targets. Source filenames renamed from v210/v210b → v292 in commit; production `supabase_migrations.schema_migrations` retains v210/v210b names. 3 ADRs: `2026-05-13-v292-definerize-financials-and-column-revoke`, `2026-05-13-v292-naming-collision-with-returns-feature`, `2026-05-13-multi-target-authorization-rule`. **v2.10 namespace is reserved for the returns/refunds/warranty feature ticket — not for security hardening.**
...
Multi-target plans that touch production state require **per-deliverable** authorization, not blanket plan-level authorization. Specifically:

- **"start [ticket]" authorizes the first deliverable only.** Each subsequent migration / deploy / cross-service mutation / git history rewrite needs its own explicit "proceed" / "go" / "commit and continue" before the agent acts. See ADR `2026-05-13-multi-target-authorization-rule.md` for the full rule and the v2.9.2 incident that surfaced it.
- **What triggers a checkpoint:** any DDL applied to the live database (`apply_migration`), force-push / branch deletion / git history rewrites, publishing a PR, sending email/Slack, hitting external APIs that mutate state.
- **What does NOT trigger a checkpoint:** local-only file edits, type-check / lint loops within a single commit's work, reads, grep, tests, internal planning (TaskCreate / Agent dispatch for research).
- **Opt-out must be explicit.** If the user says "you have authority to ship all four migrations; report at the end," that's a batch-authorization for the named scope. Inferred opt-out from a previous "go" reply is a misread.
- **At proposal time**, when the agent presents a multi-step plan touching production, the proposal should explicitly state the checkpoint cadence: *"I'll pause for re-authorization after each migration applied"* unless the user has already granted batch authority for this scope.

**Version-number discipline** (related rule, same ADR pair): security hardening that lives in the same release cycle as the prior feature version uses a `.N` increment of that version (v2.9 → v2.9.1 → v2.9.2 → v2.9.3). New feature work claims the next major-minor (v2.10, v2.11, etc.). When uncertain, ask before publishing version-tagged artifacts. The v2.9.2 work was originally drafted as "v2.10a/v2.10b" and collided with the reserved v2.10 returns/refunds feature ticket; renamed retroactively. See `decisions/2026-05-13-v292-naming-collision-with-returns-feature.md`.
