...
- **v2.9.1 AQ-24 cleanup (target v2.9.3):** rewrite the 2 pre-v2.9 views `daily_sales_7` and `expenses_by_category_mtd` to use `current_active_shop_id()` directly instead of the legacy `current_shop_id()` alias from migration 0078. AQ-24 currently allowlists these 2 views as baseline (see `design/2026-05-13-rbac-attack-surface.md` §C.3). After rewriting, drop them from the AQ-24 allowlist. (The 38 `_v28` inner functions stay allowlisted permanently per the wrap-and-rename ADR.)
- **v2.9.2 follow-ups:** (1) AQ baseline update for the 2 new `security_definer_view` advisor entries (`sale_item_financials`, `invoice_financials`); (2) drop migration 0091's permissive policies (`v29_invoices_row_read`, `v29_sale_items_row_read`) after 30-day stable observation — they're harmlessly superseded by the DEFINER-view path; (3) document the column-grant maintenance contract in CLAUDE.md (new columns on `public.sale_items`/`public.customers`/`public.inventory_batches` must explicitly `grant select` per mig 0093/0094/0095 headers).
...
## Scheduled / after pilot signal

- **v2.9.3 — extend column-grant pattern to products/variants cost columns + purchase_items cost columns.** Trigger: after pilot starts AND v2.9.2's three closed leaks (sale_items.cost_at_sale, customers.outstanding_balance, inventory_batches.cost_per_unit) prove stable in pilot. **Target 4** — `products.{cost, avg_cost, last_purchase_cost}` + `product_variants.{cost, avg_cost, last_purchase_cost}` (deprecated per ADR-0022 but still raw-API readable). Prerequisite: DEFINER-ize `product_with_default_variant` (currently `security_invoker = true`, mig 0067) with conditional projection on `view_product_cost`. **Target 5** — `purchase_items.{cost_at_purchase, overhead_per_unit, line_overhead_amount, avg_cost_before, avg_cost_after}`. Prerequisite: DEFINER-ize `purchase_item_financials` (currently `security_invoker = true`, mig 0054) with conditional projection on `view_product_cost`. Each prerequisite adds advisor lint surface (security_definer_view warning) and requires hook refactors — that's the reason v2.9.2 stopped at 3 targets and deferred these 2. Plus: any new column-leak findings surfaced during pilot use go here.
- **v2.9.3 AQ-24 cleanup:** rewrite the 2 pre-v2.9 views `daily_sales_7` and `expenses_by_category_mtd` to use `current_active_shop_id()` directly instead of the legacy `current_shop_id()` alias from migration 0078 (moved from v2.10 target per the v2.9.2 rename).
- **v2.11+ AQ-25 (one-time runtime diff between staging and prod for the v1.5–v2.3 function set).** The v2.10 chain-drift audit (`audit/2026-05-14-migration-chain-drift-audit.md`) verified zero genuinely-missing migrations by mapping prod's schema_migrations to local files, plus a 3-function spot-check (`normalize_product_text`, `touch_updated_at`, `ledger_entries_update_balance`) confirming md5 equality between staging and prod. AQ-25 generalises that spot-check: a one-time diff of `pg_get_functiondef()` between staging and prod for every v1.5–v2.3 function (~30+ functions). Build the query as `select proname, md5(pg_get_functiondef(oid)) from pg_proc where pronamespace='public'::regnamespace and proname in (...);` run against both projects, diff the md5 lists. Any mismatch is an investigation lead — staging should reproduce prod bit-for-bit unless a consolidation skipped content. Defer until staging-vs-prod-divergence becomes operationally relevant (probably never — but the discipline check is cheap). Not part of the recurring AQ suite; runs once to validate the chain assumption.
