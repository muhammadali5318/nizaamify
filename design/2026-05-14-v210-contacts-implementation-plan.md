...
11 ADRs filed during Phase C (one per locked decision).
...
| `0098_v210_add_contact_id_to_dependent_tables.sql` | `ALTER TABLE invoices/ledger_entries/purchases/inventory_batches ADD COLUMN contact_id` (nullable initially); no drops yet | Pre-auth required |
...
| `0100_v210_collateral_schema_changes.sql` | `ledger_entries` rename + add direction; `purchases` add amount_paid + outstanding; `shops` add cap column; `invoices`/`inventory_batches` column rename; `customer_balance_reconciliation` view rebuild | Pre-auth required |
...
| `0104_v210_retire_legacy_rpcs_and_v28_shims.sql` | DROP retired RPCs and 14 `_v28` shims per B.6 narrow scope. Drop `customers_view`, `suppliers_view`, `customer_outstanding` (rebuilt in 0102). | Pre-auth required |
