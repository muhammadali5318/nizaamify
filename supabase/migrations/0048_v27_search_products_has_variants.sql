-- v2.7 — extend search_products to return has_variants + variant aggregates.
--
-- The POS picker needs to know per row whether to render "+" (single-variant)
-- or "..." (multi-variant) without a second round trip. Same for the list page,
-- which v2.7 wants to show "X variants" + price-range badge.
--
-- Return columns added (appended at the end so callers reading by name don't
-- break):
--   has_variants, variant_count, min_price, max_price
--
-- For single-variant products: has_variants = false, variant_count = 1,
-- min_price = max_price = stock-variant's price.
-- For multi-variant products: has_variants = true, stock/price/avg_cost on
-- the row are NULL (no default variant), min/max derived from product_variants.
